// macro.js  — Node 18+ (has global fetch)
const path = require('node:path');

// Load .env that sits next to this file, regardless of where you run Node from
require('dotenv').config({ path: path.join(__dirname, '.env') });

const USDA_KEY = process.env.USDA_API_KEY;
if (!USDA_KEY) {
  console.error('USDA_API_KEY not found. Create .env with:\nUSDA_API_KEY=YOUR_REAL_FDC_KEY');
  process.exit(1);
}
